...
Defining a parameter with the attributes `%mutable %unique %key` is very similar to using attribute `%unique`, but not exactly the same. A mutable unique key is just a unique value that can be changed with the `%key` attribute set. Parameters with the `%key` attribute set will for example be returned in an add response when an instance is added to the data model and in a get instances response. It is important that the right information is returned after an add or get instances request, so you should always check the specifications to see whether a parameter is a key or not.

Defining a parameter with attributes `%mutable %key` makes the parameter part of a composite key. A composite key is the combination of multiple parameters that can identify the instance, but defining it as `%mutable` that parameter which is part of the composite key can be changed.
